describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const button = await prodItem.evaluateHandle(el => el.shadowRoot.querySelector('button'));
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
        ...
        document.querySelectorAll('product-item')[index]
          .shadowRoot
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (let i = 0; i < prodItems.length; i++) {
      const button = await prodItems[i].evaluateHandle(el =>
        el.shadowRoot.querySelector('button')
      );

      const textHandle = await button.evaluate(el => el.innerText);
      expect(textHandle).toBe("Add to Cart");
    }
